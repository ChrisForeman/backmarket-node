...
class BackMarketAPI {
     ...
     */
    nextTarget(high, low, target, exists) {
        if (exists) {
     ...
     */
    getLastBuyBoxPage(estPages, maxPages) {
        return __awaiter(this, void 0, void 0, function* () {
            let high = maxPages !== null && maxPages !== void 0 ? maxPages : Number.MAX_SAFE_INTEGER;
            let low = 1;
            let targetPage = estPages !== null && estPages !== void 0 ? estPages : 10;
            const pages = [];
            const validFailedPages = new Set(); //Failed pages that exist.
            while (high - low > 1) {
                let pageExists = true;
                yield this.getBuyboxData(targetPage).then(data => {
                    pages.push(data);
                }).catch(err => {
                    if (err.code === _1.ErrorCode.tooManyCalls) { //Only error code that proves the page exists.
     ...
     */
    getAllBuyboxData(estPages, maxPages) {
        return __awaiter(this, void 0, void 0, function* () {
            const results = yield this.getLastBuyBoxPage(estPages, maxPages);
            //Fill in pages that weren't retrieved when calculating the valid page range.
            const pageNums = Array.from(Array(results.lastPage).keys());
            pageNums.shift(); //Start page at one and remove an element.
            const requestedPages = Object.assign({}, results.validFailedPages);
            results.pages.forEach(page => { requestedPages.add(page.pageNumber); });
            yield Promise.all(pageNums.map(pageNum => {
     ...
     */
    updateListing(listingID, fields) {
        return axios_1.default.post(`${this.rootEndpoint}/ws/listings/${listingID}`, fields, { headers: this.headers }).then(() => {
